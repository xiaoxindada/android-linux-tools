...
class QCSparse:
    ...
    def readheader(self, offset):
        ...
        if self.file_hdr_sz != 28:
            self.error(
                "The file header size was expected to be 28, but is %u." % self.file_hdr_sz)
            return False
        if self.chunk_hdr_sz != 12:
            self.error(
                "The chunk header size was expected to be 12, but is %u." % self.chunk_hdr_sz)
            return False
    ...
    def get_chunk_size(self):
        ...
        if chunk_type == 0xCAC1:
            ...
            if data_sz != 4:
                self.error(
                    "Fill chunk should have 4 bytes of fill, but this has %u" % data_sz)
                return -1
            else:
            ...
            if data_sz != 4:
                self.error(
                    "CRC32 chunk should have 4 bytes of CRC, but this has %u" % data_sz)
                return -1
            else:
...
def key_update(iv1, asbox):
    ...
    for f in range(asbox[0x3c] - 2):
        ...
        e = gsbox(((i >> 0x10) & 0xff) * 8 + 2) ^ gsbox(((h >> 8) & 0xff) * 8 + 3) ^ \
            gsbox((a >> 0x18) * 8 + 1) ^ gsbox((e & 0xff)
                                               * 8) ^ asbox[g]  # B67F2106, 82508918
    ...
    return [(gsbox(((i >> 0x10) & 0xff) * 8) & 0xff0000) ^ (gsbox(((h >> 8) & 0xff) * 8 + 1) & 0xff00) ^
            (gsbox((a >> 0x18) * 8 + 3) &
             0xff000000) ^ gsbox((e & 0xff) * 8 + 2) & 0xFF ^ asbox[g],
            (gsbox(((a >> 0x10) & 0xff) * 8) & 0xff0000) ^ (gsbox(((i >> 8) & 0xff) * 8 + 1) & 0xff00) ^
            (gsbox((e >> 0x18) * 8 + 3) &
             0xff000000) ^ (gsbox((h & 0xff) * 8 + 2) & 0xFF) ^ asbox[g + 3],
            (gsbox(((e >> 0x10) & 0xff) * 8) & 0xff0000) ^ (gsbox(((a >> 8) & 0xff) * 8 + 1) & 0xff00) ^
            (gsbox((h >> 0x18) * 8 + 3) &
             0xff000000) ^ (gsbox((i & 0xff) * 8 + 2) & 0xFF) ^ asbox[g + 2],
        ...
        for ptr in range(0, length, 0x10):
            ...
            if pos < 0x10:
                ...
                tmp = [rkey[i] ^ int.from_bytes(
                    inp[pos + (i*4) + ptr:pos + (i*4) + ptr + 4], "little") for i in range(0, slen)]
                outp.extend(b"".join(tmp[i].to_bytes(
                    4, 'little') for i in range(0, slen)))
                if encrypt:
                    rkey = tmp
                else:
                    rkey = [int.from_bytes(
            ...
            if child.tag == "SAHARA":
                for item in child:
                    if item.tag == "File":
                        ...
                        decryptfile(key, filename, path,
                                    wfilename, start, length)
                        ...
                        for subitem in item:
                            if "filename" in subitem.attrib:
                                ...
                                if wfilename == "":
                                    continue
                                start = int(
                                    subitem.attrib["FileOffsetInSrc"]) * 0x200
                                slength = int(
                                    subitem.attrib["SizeInSectorInSrc"]) * 0x200
                                length = int(subitem.attrib["SizeInByteInSrc"])
                                sha256 = subitem.attrib["Sha256"]
                                copyfile(filename, path,
                                         wfilename, start, length)
                                csha256 = calc_digest(
                                    os.path.join(path, wfilename))
                                if sha256 != csha256 and not sparse:
                                    print("Sha256 fail.")
            # else:
        ...
        with open(outfilename, 'wb') as wf:
            ...
            for child in root:
                ...
                if child.tag == "SAHARA":
                    for item in child:
                        if item.tag == "File":
                            item, pos = encryptitem(
                                key, item, directory, pos, wf)
                elif child.tag == "UFS_PROVISION":
                    for item in child:
                        if item.tag == "File":
                    ...
                    for item in child:
                        if "filename" in item.attrib:
                            ...
                            for subitem in item:
                                subitem, pos = copyitem(
                                    subitem, directory, pos, wf)
            try:
                configpos = pos // 0x200
                ...
                with open(outfilename, 'rb') as rt:
                    with open("md5sum_pack.md5", 'wb') as wt:
                        mt = hashlib.md5()
                        mt.update(rt.read())
                        wt.write(bytes(mt.hexdigest(), 'utf-8') + b" " +
